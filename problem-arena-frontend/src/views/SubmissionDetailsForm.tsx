...
export default function SubmissionDetailsForm() {
    ...
    const userNameList = userList.map((el) => {
       return (
           <a className="panel-block" key={el.id} onClick={() => {setUserId(el.id); setSelectedUserName(el.last_name)}}>
               {el.last_name}
           </a>
       );
    });
    ...
    const problemNameList = problemList.map((el) => {
        return (
            <a className="panel-block" key={el.id} onClick={() => {setProblemId(el.id); setSelectedProblem(el.name)}}>
                {el.name}
            </a>
        );
    });
